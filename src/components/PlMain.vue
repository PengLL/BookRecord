...
<script>
	...
	export default{
		...
		methods:{
			showBookDialog(){
				this.$store.dispatch("showBookDialog");
			},
			showProgressDialog(index){
				console.log(event);
				event.preventDefault();
				this.$store.dispatch("showProgressDialog",index);
			},
			goToPlBookDetail(index){
				router.push({path:"/bookDetail/"+index});
			},
			initScroll(){
				if(!this.scroll){
